//
//  PagerTabViewSampleSample.swift
//  SelfManagement
//
...
internal class PagerTabViewSample: UIView {
    ...
    fileprivate var cellForSize: PagerTabCell!
    ...
    init(isInfinity: Bool, option: TabPageOption) {
       super.init(frame: CGRect.zero)
        ...
        Bundle(for: PagerTabCell.self).loadNibNamed("PagerTabCell", owner: self, options: nil)
        ...
        let bundle = Bundle(for: PagerTabViewSample.self)
        let nib = UINib(nibName: PagerTabCell.cellIdentifier(), bundle: bundle)
        collectionView.register(nib, forCellWithReuseIdentifier: PagerTabCell.cellIdentifier())
        cellForSize = nib.instantiate(withOwner: self, options: nil).first as! PagerTabCell
...
extension PagerTabViewSample {
    ...
    func scrollCurrentBarView(_ index: Int, contentOffsetX: CGFloat) {
        ...
        if let currentCell = collectionView.cellForItem(at: currentIndexPath) as? PagerTabCell, let nextCell = collectionView.cellForItem(at: nextIndexPath) as? PagerTabCell {
    ...
    fileprivate func moveCurrentBarView(_ indexPath: IndexPath, animated: Bool, shouldScroll: Bool) {
        ...
        if let cell = collectionView.cellForItem(at: indexPath) as? PagerTabCell {
            currentBarView.isHidden = false
...
extension PagerTabViewSample: UICollectionViewDataSource {
    ...
    internal func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PagerTabCell.cellIdentifier(), for: indexPath) as! PagerTabCell
        configureCell(cell, indexPath: indexPath)
        return cell
    }

    fileprivate func configureCell(_ cell: PagerTabCell, indexPath: IndexPath) {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if let cell = cell as? PagerTabCell, layouted {
            let fixedIndex = isInfinity ? indexPath.item % pageTabItemsCount : indexPath.item
            cell.isCurrent = fixedIndex == (currentIndex % pageTabItemsCount)
        }
    }
...
extension PagerTabViewSample: UICollectionViewDelegate {
...
extension PagerTabViewSample: UICollectionViewDelegateFlowLayout {
