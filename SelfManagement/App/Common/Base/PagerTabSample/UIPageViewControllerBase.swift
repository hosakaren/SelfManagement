...
open class UIPageViewControllerBase: UIPageViewController {
    ///　無限スクロール(true: 有限、false:無限)
    open var isInfinity: Bool = true
    // ???
    ...
    // tabview
    lazy fileprivate var tabView: PagerTabViewSample = self.configuredTabView()
...
public extension UIPageViewControllerBase {
...
// MARK: - View
extension UIPageViewControllerBase {
    ...
    // PagerTab設定
    fileprivate func configuredTabView() -> PagerTabViewSample {
        let tabView = PagerTabViewSample(isInfinity: isInfinity, option: option)
        ...
        let top = NSLayoutConstraint(item: tabView,
                                     attribute: .top,
                                     relatedBy: .equal,
                                     toItem: safeAreaGuide.topAnchor,
                                     attribute: .bottom,
                                     multiplier:1.0,
                                     constant: 0.0)
...
extension UIPageViewControllerBase: UIScrollViewDelegate {
